...
def get_wave_datasets_timeslabs(paths, splits = (0.8, 0.2), n_points=100, device='cpu', time_frac=1.0):
    assert time_frac <= 1.0 and time_frac > 0.0, 'time_frac must be between 0 and 1'
    ...
    t_all_idxs = np.arange(int(time_frac * len(t)))
...
if __name__ == '__main__':
    ...
    ds_train, ds_valid = get_wave_datasets_timeslabs(paths, time_frac=0.5)
